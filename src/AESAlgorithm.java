...
public abstract class AESAlgorithm {
    ...
    public static List<int[][]> generateRoundsKeys(String rawKey) {
        roundsKeys.add(keyExpansion(generateKey(rawKey.split(","))));
        IntStream.range(0, 9).forEach(x -> roundsKeys.add(generateRoundKey(x)));
        return roundsKeys;
    }
    ...
    private static int[][] generateRoundKey(int roundKeyIdx) {
        ...
        //Precisa rearrajar colunas e linhas
        newRoundKey[0] = xorWords(getW(lastRoundKey, 0), xorWords(subWord, roundConstant(roundKeyIdx)));
        for (int i = 1; i < lastRoundKey.length; i++) {
            newRoundKey[i] = xorWords(lastRoundKey[i], newRoundKey[i-1]);
        }
        return newRoundKey;
    }
